...
void main()
{
	...
	vec3 sin_position = vec3(position.x/30+cos(uni_phase*random_number)*25,sin(uni_phase*random_number)*25,position.z);
    //vec3 sin_position = vec3(position.x,position.y,position.z);
	vec4 world_position =  vec4(sin_position,1.0);
	...
	//float distance = 1000;//length(position_relative_to_camera.xyz);
	//visibility = 1000;// clamp(exp(-pow((distance * density), gradient)), 0.0, 1.0);
}