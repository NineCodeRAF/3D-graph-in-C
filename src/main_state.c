...
void main_state_init(GLFWwindow *window, void *args, int width, int height)
{
    ...
    rafgl_raster_load_from_image(&water_normal_raster, "res/images/white.png");
...
*/
    ...
    for(y = 0; y < grid_height; y++)
    {
        for(x = 0; x < grid_width; x++, v +=6)
        {
            ...
            grid_vertices[v + 0].normal = getNormal(-0.5f * grid_width + x + 1.0f,-0.5f * grid_height + y       , &heightMap_raster);
            grid_vertices[v + 1].normal = getNormal(-0.5f * grid_width + x + 1.0f, -0.5f * grid_height + y  +1.0f, &heightMap_raster);
            ...
            grid_vertices[v + 0].random_number = (grid_vertices[v + 0].position.z - 8.0f) / 15;
            grid_vertices[v + 1].random_number = (grid_vertices[v + 1].position.z - 8.0f) / 15;
            grid_vertices[v + 2].random_number = (grid_vertices[v + 2].position.z - 8.0f) / 15;

            grid_vertices[v + 3].random_number = (grid_vertices[v + 3].position.z + 20.0f) / 15;
            grid_vertices[v + 4].random_number = (grid_vertices[v + 4].position.z + 20.0f) / 15;
            grid_vertices[v + 5].random_number = (grid_vertices[v + 5].position.z + 20.0f) / 15;

        }
    }


    shader_program_id = rafgl_program_create_from_name("shader");
